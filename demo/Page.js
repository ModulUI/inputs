...
import { ConfirmPopup, ContentPopup } from "../src/dialogs";
...
export default class Page extends Component {
	...
	render() {
		const { dateFrom, dateTo, selected, notifications, message } =
			this.state || {};
		return (
			<section className="section_content full_width">
					...
					<form role="form" onSubmit={this.handleSubmit}>
						...
						<div className="form_group">
							<span class="label label-danger">
								Date format:{" "}
								<DateFormat value={new Date()} format="d mmmm:R yyyy HH:ss" />
							</span>
						</div>
						<div className="form_group">
							<span class="badge badge-secondary">
								Amount format: <AmountFormat value={100000.12} />
							</span>
						</div>
						<div className="form_group">
							<span class="badge badge-secondary">
								Number format: <NumberFormat value={100000.12} />
							</span>
						</div>
					</form>
				</div>
				<div className="m_top_10">
					<h3>Попапы</h3>
					<div>
						<button class="button " onClick={::this.handleOpenPopup}>
							Confirm dialog
						</button>
						<button
							class="button second"
							style={{ marginLeft: "10px" }}
							onClick={::this.handleOpenContentPopup}
						>
							Content dialog
						</button>
						<ConfirmPopup
							ref={p => (this.confirm = p)}
							text={"Some text"}
							okName={"ОК"}
							cancelName={"Cancel"}
						/>
						<ContentPopup ref={p => (this.content = p)} closeName={"Закрыть"}>
							...
							<button class="btn btn-primary" onClick={::this.handleAlert}>
								Алерт
							</button>
						</ContentPopup>
					</div>
				...
				<div className="m_top_20">
					...
					<div>
						<button
							class="button small second"
							onClick={() =>
								this.handleShowNotify(
									notifyFactory.create({
						...
						<button
							...
							style={{ marginLeft: "10px" }}
							onClick={() =>
								this.handleShowNotify(notifyFactory.error("Error message"))
							}
						>
							Error
						</button>
						<button
							class="button small clean"
							style={{ marginLeft: "10px" }}
							onClick={() =>
								this.handleShowNotify(notifyFactory.info("Info message"))
							}
						>
							Info
						</button>
						<button
							...
							style={{ marginLeft: "10px" }}
							onClick={() =>
								this.handleShowNotify(notifyFactory.warning("Warn message"))
							}
						>
							Warn
						</button>
				...
				<div className="m_top_20 m_bot_100">
					...
					<span data-mtip="test6">
						динамический контент с возможность сфокуситься на тултипе
					</span>
					<ModulTooltip
						placement="bottom"
