...
const getPlainNumber = value => {
    ...
    return tempNumber.replace(/[^0-9]+/g, '').substring(0, 4);
};

class PassportSeriesInput extends React.Component {
    ...
    parseValue(value) {
        ...
        if (plainNumber.length > 2) { number = `${ plainNumber.substring(0, 2) } ${ plainNumber.substring(2, 4) }`; }
        return number;
    }
