...
class DatePickerRange extends React.Component {
	...
	render() {
		...
		const dateFromStr = dateFrom
			? dateHelper.dateFormat(dateFrom, "d mmmm:R")
			: "";
		...
		if (!placeholder && (dateFrom || dateTo)) {
			title = "";
			if (dateFrom) title = `с ${dateFromStr} `;
			if (dateTo) title += `по ${dateToStr}`;
			if (dateFromStr === dateToStr) title = `за ${dateFromStr}`;
		}

		return (
			<Drop drop={{ position: position }} setInstance={::this.initDropInstance}>
				<a className={"drop-target icon-date drop-input " + className}>
					{title}
				</a>
						...
						<ul className="drop-menu">
							{list.map((item, i) => (
								<li key={i}>
									<a
										data-close="true"
										onClick={() => this.handleSelectPeriod(PERIOD[item])}
									>
										{PERIOD_LABEL[item]}
									</a>
								</li>
